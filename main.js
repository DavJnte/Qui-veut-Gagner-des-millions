...
const gameOver = () => {
  ...
  gameStatusContainer.textContent = points <= 1 ? `Mauvaise Réponse. Vous avez répondu juste à ${points} question.` : `Mauvaise Réponse. Vous avez répondu juste à ${points} questions.`;
...
const correctAnswerFunc = () => {
  ...
  if (points < 12) {
    ...
    gameStatusContainer.classList.remove("hidden");
    gameContainer.classList.add("hidden");
    gameStatusContainer.textContent = "Bonne Réponse !";
    pointsContainer.textContent = `${points} / 12`;
  } else {
    stopTimerMusic();
    correctAnswerAudio.play();
    ...
    gameStatusContainer.textContent =
      "FÉLICITATION ! VOUS ETES MILLIONNAIRE !";
    pointsContainer.textContent = `${points} / 12`;
  }
};
...
const timer = () => {
  ...
  intervalId = setInterval(() => {
    ...
    countDownClock.style.fontSize = "24px";
    if (interval === 0) {
      gameState = false;
      clearInterval(intervalId);
      gameOver();
